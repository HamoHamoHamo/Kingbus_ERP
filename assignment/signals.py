...
from .models import OldAssignmentConnect
...
@receiver(post_save, sender=OldAssignmentConnect)
def save_connect(sender, instance, created, **kwargs):
    ...
    try:
        ...
        allowance = OldAssignmentConnect.objects.filter(member_id=member).filter(type=type).filter(start_date__startswith=month).aggregate(Sum('allowance'))
...
@receiver(post_delete, sender=OldAssignmentConnect)
def delete_connect(sender, instance, **kwargs):
    ...
    try:
        member = instance.member_id
        salary = Salary.objects.filter(member_id=member).get(month=month)
        allowance = OldAssignmentConnect.objects.filter(member_id=member).filter(start_date__startswith=month).aggregate(Sum('allowance'))
