...
from .models import Consulting, VehicleInspectionRequest
...
class ConsultingList(generic.ListView):
    ...
    def get_queryset(self):
        ...
        if date1 > date2:
            raise Http404
        consulting_list = Consulting.objects.select_related('member_id').filter(date__gte=date1).filter(date__lte=date2)
...
class MaintanceList(generic.ListView):
    template_name = 'complaint/vehicle_inspection.html'
    context_object_name = 'vehicle_inspection_list'
