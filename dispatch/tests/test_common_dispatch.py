...
from common_test import LoginTestCase
...
from common_constant import TODAY
...
class EditCheckViewTest(LoginTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.regularly_data = DispatchRegularlyData.objects.create(group=cls.group)
        regularly = DispatchRegularly.objects.create(group=cls.group, regularly_id=cls.regularly_data)
        cls.regularly_connect = DispatchRegularlyConnect.objects.create(regularly_id=regularly, departure_date=cls.DEPARTURE_DATE1, arrival_date=cls.ARRIVAL_DATE1, bus_id=cls.bus, driver_id=cls.member)
        ...
        cls.set_view_type(cls.regularly_data.id)

    @classmethod
    def set_view_type(self, id):
        self.test_id = id
        self.test_url = reverse('dispatch:regularly_route_edit_check')
