...
class DataCollector:
    ...
    def get_connects_time_list(self, date):
        ...
        for connect in daily_connects:
            ...
            if is_time_difference_under_90:
                ...
                if i == length - 1 or daily_connects[i + 1]['work_type'] == '일반' or daily_connects[i + 1]['work_type'] == '업무':
    ...
    def check_arrival_can_parking_outside(self, i, time_list, length, daily_connects):
        # 첫 운행일 경우에만 외부 주차 가능 여부 확인
        if (i == length - 1 or daily_connects[i + 1]['work_type'] == '일반' or daily_connects[i + 1]['work_type'] == '업무') and self.member.can_parking_outside:
